...
public struct Elements: Codable, Equatable, Sendable {
    ...
    let dragCoeff: Double // Ballistic coefficient.
    var n₀ʹ: Double = 0.0 // Kozai'd version (keep for JSON encoding)
}
...
public extension Elements {
    ...
    init(_ line0: String, _ line1: String, _ line2: String) throws {
        ...
        noradIndex = alpha5ID(stringlet.trimmingCharacters(in: .whitespaces))

        tleClass = String(bytes: lineOneBytes[7 ... 7], encoding: .utf8)!
        ...
        launchName = "\(launchYear < 57 ? 2000 : 1900 + launchYear)-\(launchPart)"
        ...
        t₀ = epochDays(year: (epochYear < 57 ? 2000 : 1900) + epochYear, days: Double(stringlet)!)
        ...
        guard noradIndex == alpha5ID(stringlet.trimmingCharacters(in: .whitespaces)) else {
        ...
        i₀ = Double(stringlet.trimmingCharacters(in: .whitespaces))! * deg2rad

        stringlet = String(bytes: lineTwoBytes[17 ... 24], encoding: .utf8)!
        Ω₀ = Double(stringlet.trimmingCharacters(in: .whitespaces))! * deg2rad

        stringlet = "." + String(bytes: lineTwoBytes[26 ... 32], encoding: .utf8)!
        e₀ = Double(stringlet.replacingOccurrences(of: " ", with: "0"))!

        stringlet = String(bytes: lineTwoBytes[34 ... 41], encoding: .utf8)!
        ω₀ = Double(stringlet.trimmingCharacters(in: .whitespaces))! * deg2rad

        stringlet = String(bytes: lineTwoBytes[43 ... 50], encoding: .utf8)!
        M₀ = Double(stringlet.trimmingCharacters(in: .whitespaces))! * deg2rad

        stringlet = String(bytes: lineTwoBytes[52 ... 62], encoding: .utf8)!
        n₀ʹ = Double(stringlet.trimmingCharacters(in: .whitespaces))!

        stringlet = String(bytes: lineTwoBytes[63 ... 67], encoding: .utf8)!
        revNumber = Int(stringlet.trimmingCharacters(in: .whitespaces))!

        unKozai(n₀ʹ * (π / 720.0))

        guard ephemType == 0 || ephemType == 2 || ephemType == 3 else {
            throw SatKitError.TLE(tleError: "Line1 ephemerisType ≠ 0, 2 or 3 .. [\(ephemType)]")
        }
    }
}
...
extension Elements {
    /* ╭╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╮
     ...
     ╰╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╯ */
    mutating func unKozai(_ n₀ʹ: Double) {
        do {
            let θ = cos(i₀) //         cos(i₀)  ..  θ
...
public extension Elements {
    ...
    func debugDescription() -> String {
        ...
        return String(format: """
                      ...
                      commonName.padding(toLength: 24, withPad: " ", startingAt: 0),
                      noradIndex,
                      launchName.padding(toLength: 11, withPad: " ", startingAt: 0),
                      revNumber, tleNumber,
                      String(describing: Date(ds1950: t₀)), t₀,
                      i₀ * rad2deg, ω₀ * rad2deg, n₀ / (π / 720.0), Ω₀ * rad2deg,
                      M₀ * rad2deg, e₀, dragCoeff)
    }
}
...
public func formatOK(_ line1: String, _ line2: String) -> Bool {
    ...
    guard lineOneRegEx.numberOfMatches(in: line1,
                                       range: NSRange(location: 0, length: 69)) == 1
    else {
